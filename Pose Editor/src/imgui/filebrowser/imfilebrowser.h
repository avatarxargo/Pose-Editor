...
namespace ImGui
{
	...
	public:
		...
		/// modification:
		/// <desc>Sets the current typed text to the provided value</desc>
		/// <date>9/5/2021</date>
		/// <version>1.0</version>
		/// <author>David Hrusa</author>
		/// <email>hrusadav@gmail.com</email>
		void SetInputText(std::string filename);
...
inline void ImGui::FileBrowser::SetInputText(std::string filename)
{
	strcpy_s(
		inputNameBuf_->data(), inputNameBuf_->size(),
		filename.c_str());
